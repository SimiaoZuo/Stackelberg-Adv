...
class TransformerModel(FairseqEncoderDecoderModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--init-std', type=float, default=1.0,
                            help='Std to initialize perturbations')
        ...
        parser.add_argument('--mode', type=str, default=None,
                            help='unroll for unrolling, standard for standard adversarial training')
...
class TransformerEncoder(FairseqEncoder):
    ...
    def forward_embedding(self, src_tokens, perturbation=None, adv_step=-1):
        # embed tokens and positions
        if adv_step > 0 and perturbation is None:
            perturbation = self.perturbation_enc
        x = embed = self.embed_scale *\
                    self.embed_tokens(src_tokens, perturbation=perturbation,
    ...
    def forward(self,
        ...
        """
        x, encoder_embedding = \
            self.forward_embedding(src_tokens, perturbation=perturbation, adv_step=adv_step)
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def extract_features_scriptable(
        ...
        x = embed = self.embed_scale *\
            self.embed_tokens(prev_output_tokens, perturbation=perturbation,
...
class Embedding(nn.Embedding):
    ...
    def forward(self, x, perturbation=None, adv_step=-1, enc_or_dec=-1):
        """
        Input:
            x: Input indices.
