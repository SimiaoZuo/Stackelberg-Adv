...
from fairseq.criterions.adv_unroll_loss import compute_unroll_loss, reduce_adv_unroll_metrics
...
class FairseqTask(object):
    ...
    def train_step(
        ...
        if mode not in ['standard', 'unroll']:
            raise NotImplementedError('Invalid mode.')
        if mode == 'unroll' and use_reg > 0 and not ignore_grad:
            loss_scale = None  # loss scale for FP16 training
            if hasattr(optimizer, 'scaler') and optimizer.scaler is not None:
                loss_scale = optimizer.scaler.loss_scale
            loss_adv = compute_unroll_loss(model, sample, net_output,
                                           padding_idx=criterion.padding_idx, loss_scale=loss_scale)
    ...
    def reduce_metrics(self, logging_outputs, criterion):
        ...
        if self.args.mode == 'unroll':
            reduce_adv_unroll_metrics(logging_outputs)
        else:
            raise NotImplementedError('Invalid mode.')
